...
const App = () => {
  ...
  const propertyLayout = layouts[LayoutPaths.PropertyPanel] as PropertyLayout ?? { pages: [] };
  const propertyRoutes = propertyLayout.pages.map((page: PropertyPage) => (
    <Route key={page.id} path={page.id} element={<PropertySheet page={page} />} />
  ));  
  ...
  return (
    <MemoryRouter initialEntries={route}>
      <Routes>
        ...
        <Route path="/diagram-panel" element={viewWithProviders(<DiagramView />)}/>
        <Route path="/property-panel/*" element={

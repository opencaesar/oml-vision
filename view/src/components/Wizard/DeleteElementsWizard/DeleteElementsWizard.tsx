...
function DeleteElementsWizard({
  ...
  const [cascadingRelations, setCascadingRelations] = useState<ITableData[]>([]);
  const [IRIsToDelete, setIRIsToDelete] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const handler = (event: MessageEvent) => {
      ...
      switch (message.command) {
        case Commands.LOADED_ELEMENT_DEPENDENCIES:
          specificMessage = message as CommandStructures[Commands.LOADED_ELEMENT_DEPENDENCIES]
          ...
          try {
            ...
            const relationIRIs = results.relationIRIs || [];
            // Check if any row has canDelete set to false
            if (relationIRIs.find((row: ITableData) => row.canDelete === false)) {
              setCanDeleteElements(false);
            }
            const IRIsToDelete = results.IRIsToDelete || [];
            setCascadingRelations(relationIRIs);
            setIRIsToDelete(IRIsToDelete);
          } catch (error) {
            let errorMessage = '';
        ...
        case Commands.DELETED_ELEMENTS:
          specificMessage = message as CommandStructures[Commands.DELETED_ELEMENTS];
    ...
    >
      <div className="flex flex-col items-center justify-center h-full w-full">
        <div className="flex w-full items-center justify-between mb-4">
          <h1 className="modal-wizard-header pl-2.5 text-[var(--vscode-foreground)]">Delete</h1>
          <button disabled={isDeleting }type="button" onClick={() => closeWizard("DeleteElementsWizard")} className="bg-transparent rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center text-[var(--vscode-icon-foreground)] hover:bg-[var(--vscode-toolbar-hoverBackground)]">
        ...
        {!isLoading && !isDeleting && !errorMessage ? (
          <div className="flex justify-left text-xs w-full px-2.5 py-1.5">
            The following elements will be deleted:
          </div>
        ) : <div className="pt-4"></div>}
        <div className="flex justify-center space-x-4 w-full px-2.5 pb-4">
          {(isLoading || isDeleting) ? <Loader /> : errorMessage ? (
            <p className='text-[color:var(--vscode-foreground)]'>{errorMessage}</p>
          ) : (
            <div className="delete-tree-container flex w-full h-[40vh] overflow-y-auto overflow-x-hidden bg-[color:var(--vscode-panel-background)] rounded-md">
              <FillFlexParent headerHeight={0}>
                {(dimens) => (
                  <ArboristTree
                    {...dimens}
                    initialData={cascadingRelations}
        ...
        {!isLoading && !canDeleteElements && (
          <div className="flex justify-center text-center text-[var(--vscode-charts-red)] text-xxs w-full px-2.5 py-2.5">
            <strong>
              Elements can't be deleted, as some are either Baseline, Retracted, or Deprecated.
            </strong>
          </div>
        )}
        <div className="flex justify-between items-center space-x-4 w-full px-2.5 mt-4 mb-2.5">
          <div>
            {errorMessage !== "" && (
              <VSCodeButton appearance="primary" className="rounded-sm" onClick={refetchRelations}>
                Refetch Relations
          ...
          <div className="flex space-x-4">
            <VSCodeButton disabled={isDeleting} appearance="secondary" type="button" onClick={() => closeWizard("DeleteElementsWizard")} className="rounded-sm">Cancel</VSCodeButton>
            <VSCodeButton className="rounded-sm bg-[var(--vscode-errorForeground)]" onClick={onSubmit} disabled={isLoading || isDeleting || errorMessage !== "" || !canDeleteElements}>
              Delete
...
function Node({ node, style, dragHandle }: NodeRendererProps<ITableData>) {
  ...
  const maturity = node.data.maturity;
  ...
  return (
    ...
    >
      <div className={'flex p-[var(--st-table-th-padding)] items-center'}>
        ...
        <span className={`text ${maturity ? getLifecycleStateStyles(maturity) : ''}`}>
          {node.data.name}
        </span>
      </div>
    </div>

...
import { IRowMapping, DataLayoutsType, DiagramLayout, IDiagramRowMapping, IDiagramEdgeMapping, IDiagramOverlay } from "../../interfaces/DataLayoutsType";
import {
  Edge,
  MarkerType,
  Node,
...
export const mapDiagramValueData = (
  ...
  let filteredData: { [key: string]: ITableData[] } = JSON.parse(JSON.stringify(data));

  if (filterObject && Object.keys(filterObject).length > 0) {
    Object.entries(filterObject).forEach(([filterType, filteredIris]) => {
      filteredData[filterType] = (filteredData[filterType] || []).filter((row) => {

        // Get all properties ending with 'Iri'
        const iriProps = Object.keys(row).filter(prop => prop.endsWith('Iri'));
    ...
    const allowedKeys = new Set([...Object.keys(filterObject), ...overlayIds, ...edgeIds]);
  ...
  const processEntry = (entry: ITableData, rowMapping: IDiagramRowMapping | IDiagramOverlay, id: string, parentId?: string, extent?: 'parent') => {
    const label = rowMapping.labelFormat.replace(/{([^}]+)}/g, (match, placeholder) => {
    ...
    let processedEntry: ITableData = {
      ...
      "position": { x: 0, y: 0 },
      "type": nodeType,
      "data": { label, iri, edgeKey, parentId, extent, containmentId, containedEntries }
    };
  ...
  const recursiveMapper = (
    ...
    return filteredData[rowMapping.id]
      .filter(row => {
      ...
      .map((row: ITableData, index: number) => {
        let identifier = parentIdentifier ? `${parentIdentifier}-${index}` : `${index}`;
        let processedRow = processEntry(row, rowMapping, identifier, parentIdentifier, extent);
        ...
        if (rowMapping.isRecursive) {
          children = recursiveMapper(rowMapping, 'parent', row.iri, 'parent', identifier)
        } else if (rowMapping.subRowMappings) {
          children = rowMapping.subRowMappings.flatMap((subMapping: IDiagramRowMapping) => recursiveMapper(subMapping, rowMapping.id, row.iri, 'parent', identifier));
        }
  ...
  const mapOverlayNodes = (overlay: IDiagramOverlay): ITableData[] => {
    ...
    return filteredData[overlay.id].map((row: ITableData) => processEntry(row, overlay, uuidv4()));
  }

  const mapEdges = (edgeMapping: IDiagramEdgeMapping, edgeData: ITableData[]): Edge[] => {
    let edges: Edge[] = [];
  
    edgeData.forEach((edge: ITableData) => {
      let sourceNode = flatNodes.find((node: ITableData) => node.data.edgeKey === edge[edgeMapping.sourceKey]);
      let targetNode = flatNodes.find((node: ITableData) => node.data.edgeKey === edge[edgeMapping.targetKey]);
      let flow = edge.flow || '=';
      let colorIdentifier = edge[edgeMapping.colorKey] || "";
      const label = edgeMapping.labelFormat.replace(/{([^}]+)}/g, (match, placeholder) => {
      ...
      if (sourceNode && targetNode) {
        ...
        if (colorIdentifier && !colorMap.has(colorIdentifier)) {
          colorMap.set(colorIdentifier, { color: stringToColor(colorIdentifier), isEdge: true });
        }
        
        let color = colorIdentifier ? colorMap.get(colorIdentifier).color : "var(--vscode-banner-foreground)";
        ...
        if (flow === "<" || flow === "=") newEdge.markerStart = { type: MarkerType.ArrowClosed, width: 30, height: 30 };
        if (flow === ">" || flow === "=") newEdge.markerEnd = { type: MarkerType.ArrowClosed, width: 30, height: 30 };
  ...
  const startMappingFromAnyNode = (
    ...
    if (!filterObject || (filterObject && filterObject[rowMapping.id])) {
      ...
      return rowMapping.subRowMappings.flatMap(subMapping => startMappingFromAnyNode(subMapping, filterObject));
    } else {
      return [];
    }
  }
  ...
  const overlayNodes = layout.overlays?.flatMap(overlay => mapOverlayNodes(overlay)) ?? [];
  nodes.push(...overlayNodes);
  const edges = layout.edges.flatMap(edgeLayout => mapEdges(edgeLayout, filteredData[edgeLayout.id]));
  const legendItems = Array.from(colorMap, ([label, { color, isEdge = false }]) => ({ label, color, isEdge }));
...
const isInteractiveSelector = (s: ReactFlowState) => s.nodesDraggable && s.nodesConnectable
...
const calculateNodeSize = (node: any, isHorizontal: boolean) => {
  ...
  return { width: Math.max(DEFAULT_WIDTH, totalWidth), height: Math.max(DEFAULT_HEIGHT, totalHeight) };
}

// A helper function that assigns a color to a node based on depth.
const assignNodeColor = (depth: number, isOverlay: boolean = false) => {
  // Define colors for each layer. Extend this list for more layers.
  const backgroundColors = ["#3D348B", "#7678ED", "#F7B801", "#F18701", "#F35B04"];
  const textColors = ["#FFF", "#FFF", "#000", "#FFF", "#FFF"];
  let backgroundColor = backgroundColors[depth % backgroundColors.length];
  let color = textColors[depth % textColors.length];
...
const processNodes = (nodes: ITableData, parentNodeId = null, isHorizontal = false, depth = 0) => {
  return nodes.map((node: any) => {
    ...
    const processedNode: any = {
      ...
      width,
      height,
      style: assignNodeColor(depth, node.data.isOverlay || node.type === "overlay")
    };

    if (node.children) {
      processedNode.children = processNodes(node.children, node.id, isHorizontal, depth + 1);
    } else {
...
const flattenNodes = (nodes: any, parentX: number = 0, parentY: number = 0) => {
  ...
  return nodes.reduce((flattenedNodes, node) => {
    ...
    let nodes = [node];
    if (node.children) {
      nodes = [...nodes, ...flattenNodes(node.children, node.canvasX, node.canvasY)];
...
const getOverlayBoundingBoxes = (overlayNodes: ITableData[], nodes: ITableData[]): ITableData[] => {
  return overlayNodes.reduce((acc: ITableData[], overlay: ITableData) => {
    ...
    const minX = Math.min(...includedNodes.map(node => node.canvasX)) - PADDING;
    const minY = Math.min(...includedNodes.map(node => node.canvasY)) - PADDING;
    const maxX = Math.max(...includedNodes.map(node => node.canvasX + node.width)) + PADDING;
    const maxY = Math.max(...includedNodes.map(node => node.canvasY + node.height)) + PADDING;
...
export const getLayoutedElements = (nodes: ITableData, edges: Edge[]) => {
  ...
  return elk
    ...
    .then((layoutedGraph) => {
      ...
      const {unboundedOverlayNodes, filteredFlattenedNodes} = flattenedNodes.reduce((acc: any, node: any) => {
      ...
      const overlayBoundingBoxes = getOverlayBoundingBoxes(unboundedOverlayNodes, flattenedNodes)

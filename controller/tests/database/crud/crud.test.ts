...
jest.mock("../../../src/utilities/loaders/loadSparqlConfigFiles.ts", () => {
  return {
    ...
    globalUpdateEndpoint: "http://localhost:3030/tutorial2/update",
  };
});
...
test("CRUD Queries Don't Return Blank Results", async () => {
  ...
  const updated = updateQuery(subject, predicate, object, newObject, graph);
  ...
  const update_delete_read = readQuery(subject, predicate, newObject, graph);

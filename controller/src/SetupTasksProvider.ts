...
export class SetupTasksProvider implements vscode.WebviewViewProvider {
	...
	public resolveWebviewView(
		...
		webviewView.webview.html = getHtmlForWebview(webviewView.webview, this._extensionPath, "/setup-tasks-panel");
	...
	private handleGradleTasksRequest() {
		this._gradleApi.getTaskProvider().provideTasks().then((tasks: vscode.Task[]) => {
			if (tasks) {
				...
				this._onDidLoadTasksSubscription = this._gradleApi.getTaskProvider().onDidLoadTasks((tasks: vscode.Task[]) => {
					this.sendGradleTasks(tasks);
				});
			}
		});
	...
	private sendGradleTasks(tasks: vscode.Task[]) {
		...
		const visionTasks = tasks
			.filter(task => task.definition.group === "vision" || task.name === "clean" || task.name === "build")
			.map(task => task.name);
	...
	public sendMessage<K extends keyof CommandDefinitions>(message: { command: K } & CommandDefinitions[K] ) {
		if (this._view) {
